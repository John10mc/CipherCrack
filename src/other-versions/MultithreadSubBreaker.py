...
def decrypt(cipherText, gram):
    ...
    # a queue object to allow threads to communicate and share data
    scores = Queue()
    threads = []
    # for loop for creating and starting threads. Append them to a list so they can be joined
    for i in range(8):
        key = ''.join(sample(key,len(key)))
        # create a thread with the testKey as the target to find the best key from the random key by changing
        # two letters and generating a score
        thread = Thread(target=testKey, args=(key, gram, cipherText, scores))
        threads.append(thread)
        thread.start()

    # for each thread created join them so the main thread wont complete until all
    # the sub threads complete
...
# each thread created will execute this method. it loops changing two letters of the key
